/*! highlight.js v9.18.1 | BSD3 License | git.io/hljslicense */ ! function(e) {
	...
	var u = [],
		...
		k = "</span>",
		E = "Could not find the language '{}', did you forget to load/include a language module?",
		M = {
			classPrefix: "hljs-",
			tabReplace: null,
			useBR: !1,
	...
	function _(e, t, r) {
		...
		function c(e) {
			n += "<" + f(e) + u.map.call(e.attributes, function(e) {
				return " " + e.nodeName + '="' + C(e.value).replace(/"/g, "&quot;") + '"'
			}).join("") + ">"
		}
	...
	function B(a) {
		...
		if (a.c && -1 != a.c.indexOf("self")) {
			if (!x) throw new Error("ERR: contains `self` is not supported at the top-level of a language.  See documentation.");
	...
	function R(t, e, n, r) {
		...
		function s() {
			g += (null != f.sL ? function() {
				var e = "string" == typeof f.sL;
				if (e && !w[f.sL]) return C(_);
				var t = e ? R(f.sL, _, !0, m[f.sL]) : S(_, f.sL.length ? f.sL : void 0);
				return 0 < f.relevance && (h += t.relevance), e && (m[f.sL] = t.top), c(t.language, t.value, !1, !0)
			} : function() {
				var e, t, r, a, n, i, s;
				if (!f.k) return C(_);
				for (a = "", t = 0, f.lR.lastIndex = 0, r = f.lR.exec(_); r;) a += C(_.substring(t, r.index)), n = f, i = r, s = b.cI ? i[0].toLowerCase() : i[0], (e = n.k.hasOwnProperty(s) && n.k[s]) ? (h += e[1], a += c(e[0], C(r[0]))) : a += C(r[0]), t = f.lR.lastIndex, r = f.lR.exec(_);
				return a + C(_.substr(t))
			})(), _ = ""
		}

		function o(e) {
			g += e.cN ? c(e.cN, "", !0) : "", f = Object.create(e, {
				parent: {
					value: f
				}
			})
		...
		function l(e) {
			var t = e[0],
				r = e.rule;
			return r && r.endSameAsBegin && (r.eR = new RegExp(t.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&"), "m")), r.skip ? _ += t : (r.eB && (_ += t), s(), r.rB || r.eB || (_ = t)), o(r), r.rB ? 0 : t.length
		}
		...
		function d(e) {
			...
			if (a) {
				var n = f;
				for (n.skip ? _ += t : (n.rE || n.eE || (_ += t), s(), n.eE && (_ = t)); f.cN && (g += k), f.skip || f.sL || (h += f.relevance), (f = f.parent) !== a.parent;);
		...
		function a(e, t) {
			...
			if ("begin" == u.type && "end" == t.type && u.index == t.index && "" === r) return _ += i.slice(t.index, t.index + 1), 1;
			if ("begin" === (u = t).type) return l(t);
			if ("illegal" === t.type && !n) throw new Error('Illegal lexeme "' + r + '" for mode "' + (f.cN || "<unnamed>") + '"');
			if ("end" === t.type) {
				var a = d(t);
				if (null != a) return a
			}
		...
		var b = A(t);
		if (!b) throw console.error(E.replace("{}", t)), new Error('Unknown language: "' + t + '"');
		B(b);
		var p, f = r || b,
			m = {},
			g = "";
		...
		var _ = "",
			h = 0;
		try {
			for (var v, y, N = 0; f.t.lastIndex = N, v = f.t.exec(i);) y = a(i.substring(N, v.index), v), N = v.index + y;
	...
	function v(e) {
		return M.tabReplace || M.useBR ? e.replace(r, function(e, t) {
	...
	function d(e) {
		var t, r, a, n, i, s, c, o, l, d, u = function(e) {
			...
			if (i += e.parentNode ? e.parentNode.className : "", r = p.exec(i)) {
				var s = A(r[1]);
				return s || (console.warn(E.replace("{}", r[1])), console.warn("Falling back to no-highlight mode for this block.", e)), s ? r[1] : "no-highlight"
			}
			for (t = 0, a = (i = i.split(/\s+/)).length; t < a; t++)
				if (m(n = i[t]) || A(n)) return n
		}(e);
		m(u) || (M.useBR ? (t = document.createElement("div")).innerHTML = e.innerHTML.replace(/\n/g, "").replace(/<br[ \/]*>/g, "\n") : t = e, i = t.textContent, a = u ? R(u, i, !0) : S(i), (r = g(t)).length && ((n = document.createElement("div")).innerHTML = a.value, a.value = _(r, g(n), i)), a.value = v(a.value), e.innerHTML = a.value, e.className = (s = e.className, c = u, o = a.language, l = c ? b[c] : o, d = [s.trim()], s.match(/\bhljs\b/) || d.push("hljs"), -1 === s.indexOf(l) && d.push(l), d.join(" ").trim()), e.result = {
			language: a.language,
			re: a.relevance
	...
	return n.highlight = R, n.highlightAuto = S, n.fixMarkup = v, n.highlightBlock = d, n.configure = function(e) {
		M = i(M, e)
		...
		try {
			...
			if (console.error("Language definition for '{}' could not be registered.".replace("{}", t)), !x) throw e;
			console.error(e), r = N
		}
		l(w[t] = r), r.rawDefinition = e.bind(null, n), r.aliases && r.aliases.forEach(function(e) {
			b[e] = t
		})
		...
		var r = "decltype\\(auto\\)",
			...
			p = {
				cN: "function",
				b: "((decltype\\(auto\\)|(?:[a-zA-Z_]\\w*::)?[a-zA-Z_]\\w*(?:<.*?>)?)[\\*&\\s]+)+" + l,
				rB: !0,
				e: /[{;=]/,
				eE: !0,
				k: d,
